...
test_gitstate() {
    ...
    echo "== check out state store =="
    STATE_WORKING=$(mktemp -d)
    git clone "$STATE_REMOTE" "$STATE_WORKING"
    assert_equal "0" "$?" "Failed to clone state store"

    echo "== trigger update from state store =="
    pushd "$STATE_WORKING/intent" > /dev/null
    ...
    check_file_in_remote "$STATE_REMOTE" "main" "support.txt" "state"
